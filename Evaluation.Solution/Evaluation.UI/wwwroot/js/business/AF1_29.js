...
$('#fileInput').change(function () {
    ...
    if (fileName != '') {
        ...
        formData.append('code', "AF1_29");

        $.ajax({
            type: 'POST',
            url: '/ExcelImport/ImportExcelAF1_29',
            data: formData,
            processData: false,
            contentType: false,
    ...
    <td data-annualWages="${data.annualWages}">${data.annualWages}</td>
...
$("#btnSendEmailExcel").click(function () {
    ...
    formData.append('businessPage', JSON.stringify("AF1_29"));
...
function submitAF(isSent) {
    ...
    obj.RecId = trxID;
    obj.AF1BL291908 = dataList;
    formBody.append("data", JSON.stringify(obj));
    if (trxID != 0) {

        $.ajax({
            url: "/Business/EditAF1_29",
            type: "POST",
            contentType: false,
            processData: false,
            data: formBody,
...
var table = $('#tableAf129').DataTable({
    data: datatablelist,
    responsive: true,
    autoWidth: true,
    columns: [
...
$("#btnDownloadEmptyExcel").click(function () {
    downloadEmptyExcel(BusinessLineCode, JSON.stringify("AF1_29"), JSON.stringify("AF"));
})