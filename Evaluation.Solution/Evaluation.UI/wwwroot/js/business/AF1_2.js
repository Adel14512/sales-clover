...
$('#fileInput').change(function () {
    ...
    if (fileName != '') {
        ...
        $.ajax({
            type: 'POST',
            url: '/ExcelImport/ImportExcelAF1_2',
            data: formData,
            processData: false,
            contentType: false,
    ...
    <td data-countryOfDepartureCode="${regionList.find(x => x.description.toLowerCase() == data.countryOfDepartureCode.toLowerCase()).code}">${data.countryOfDepartureCode}</td>
    <td data-countryOfDestinationCode="${territorialScopeList.find(x => x.description.toLowerCase() == data.countryOfDestinationCode.toLowerCase()).code}">${data.countryOfDestinationCode}</td>
    <td data-stayTripOptionCode="${stayTripOptionList.find(x => x.description.toLowerCase() == data.stayTripOptionCode.toLowerCase()).code}">${data.stayTripOptionCode}</td>
    <td data-nbrDaysWhenLess92="${data.nbrDaysWhenLess92}">${data.nbrDaysWhenLess92}</td>`;
...
$("#btnSendEmailExcel").click(function () {
    ...
    tableRows.forEach((row) => {
        ...
        data.countryOfDepartureCode = row.querySelector('[data-countryOfDepartureCode]').textContent;
        data.countryOfDestinationCode = row.querySelector('[data-countryOfDestinationCode]').textContent;
        data.stayTripOptionCode = row.querySelector('[data-stayTripOptionCode]').textContent;
        data.nbrDaysWhenLess92 = row.querySelector('[data-nbrDaysWhenLess92]').textContent;
...
var columnConfig = [
    ...
    { data: 'countryOfDepartureCode', title: 'Country Of Departure' },
    { data: 'countryOfDestinationCode', title: 'Countr Of Destination' },
    { data: 'stayTripOptionCode', title: 'Stay/Trip Option' },
    { data: 'nbrDaysWhenLess92', title: 'Nbr Days When Less92' }
];
...
var table = $('#tableAf12').DataTable({
    ...
    columns: [
        ...
        {
            data: 'countryOfDestinationCode', title: 'Country Of Destination',
            render: function (data, type, row) {
                return territorialScopeList.find(x => x.code == data).description || '';
            }
        },
        {
            data: 'stayTripOptionCode', title: 'Stay/Trip Option',
            render: function (data, type, row) {
                return stayTripOptionList.find(x => x.code == data).description || '';
            }
        },
        { data: 'nbrDaysWhenLess92', title: 'Nbr Days When Less92' },
...
$("#btnDownloadEmptyExcel").click(function () {
    downloadEmptyExcel(BusinessLineCode, JSON.stringify("AF1_2"), JSON.stringify("AF"));
})
...
function SubmitAF(isEmpty) {
    ...
    try {
        ...
        tableRows.forEach((row) => {
            ...
            data.countryOfDepartureCode = row.querySelector('[data-countryOfDepartureCode]').getAttribute('data-countryOfDepartureCode');
            data.countryOfDestinationCode = row.querySelector('[data-countryOfDestinationCode]').getAttribute('data-countryOfDestinationCode');
            data.stayTripOptionCode = row.querySelector('[data-stayTripOptionCode]').getAttribute('data-stayTripOptionCode');
            data.nbrDaysWhenLess92 = row.querySelector('[data-nbrDaysWhenLess92]').getAttribute('data-nbrDaysWhenLess92');


            dataList.push(data); // Add the object to the list
        });
        obj.AF1BL021502 = dataList;
    } catch {
        if (!isEmpty) {
