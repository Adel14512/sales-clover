...
$("#btnSendEmailExcel").click(function () {
    ...
    //$('#tableAf104').DataTable().rows().data().toArray().forEach((row) => {
    ...
    //});
    var formData = new FormData();
    formData.append('data', JSON.stringify($('#tableAf104').DataTable().rows().data().toArray()));
...
function submitAF(isSent) {
    ...
    obj.AF1BL041312 = $('#tableAf104').DataTable().rows().data().toArray();
    formBody.append("data", JSON.stringify(obj));
    if (trxID != 0) {
        $.ajax({
            url: "/Business/EditAF1_04",
            type: "POST",
            contentType: false,
            processData: false,
            data: formBody,
            ...
            success: function (data) {
                ...
                if (data != null && data.webResponseCommon.returnCode == '202') {
                    ...
                    if (isSent) {
                        downloadEmptyExcel(BusinessLineCode, JSON.stringify("AF1_04"), JSON.stringify("AF"));
                    } else {
...
$(document).ready(function () {
    ...
    $('#tInssurredParty tbody').on('click', 'tr', function () {
        ...
        $("#btnDownloadEmptyExcelStep4").click(function () {
            downloadEmptyExcel(BusinessLineCode, JSON.stringify("AF1_04"), JSON.stringify("BN"));
        });
        $("#btnDownloadEmptyExcelStep5").click(function () {
            downloadEmptyExcel(BusinessLineCode, JSON.stringify("AF1_04"), JSON.stringify("PL"));
        });
    ...
    function step3DataTable() {
        return $('#tStep3').DataTable({
            data: tInssurredPartySelectedData.slip3BL041312,
            responsive: true,
            autoWidth: true,
            destroy: true,
            columns: [
    ...
    function step2DataTable() {
        return $('#tStep2').DataTable({
            destroy: true,
            data: tInssurredPartySelectedData.slip2BL041312,
            responsive: false,
            autoWidth: true,
            scrollX: true,
            columns: [
...
function step4DataTable() {
    return $('#tStep4').DataTable({
        destroy: true,
        data: tInssurredPartySelectedData.slip4BL041312,
        responsive: true,
        autoWidth: false,
        columns: [
            //   { data: 'recId', title: 'ID', },
...
$("#btnSubmitStep5").click(function () {
    ...
    obj.salesTransactionBL041312DetailsPrices = tStep5Table.rows().data().toArray().map(obj => ({ ...obj, SalesTrxDetailsId: tInssurredPartySelectedData.recId }));
    formData.append('slip', JSON.stringify(obj));
    $.ajax({
        type: 'POST',
        url: '/Slip/SalesTransactionBL041312DetailsPricesNewRec',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (data, status, xhr) {
            ...
            if (data != null && data.webResponseCommon != null && data.webResponseCommon.returnCode == '200') {
                ...
                details.filter(x => x.recId == tInssurredPartySelectedData.recId)[0].slip4BL041312 = tStep4Table.rows().data().toArray();
...
function step1DataTable(tInssurredPartySelectedData, columnDefs) {
    return $('#tableAf104').DataTable({
        destroy: true,
        data: tInssurredPartySelectedData.aF1BL041312,
        responsive: true,
        autoWidth: true,
        columns: [
            { data: 'serial', title: 'Serial' },
...
$("#btnSubmitStep2").click(function () {
    ...
    $.ajax({
        type: 'POST',
        url: '/Slip/SalesTransactionBL041312DetailsUpdSlip2',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (data, status, xhr) {
            ...
            if (data != null && data.webResponseCommon != null && data.webResponseCommon.returnCode == '202') {
                details.filter(x => x.recId == tInssurredPartySelectedData.recId)[0].slip2BL041312 = tStep2Table.rows().data().toArray();
                toastr.success("", "Saved");
            } else {
                toastr.warning(data.webResponseCommon.returnMessage, "UnSaved");
            }
...
$("#btnSubmitStep3").click(function () {
    ...
    $.ajax({
        type: 'POST',
        url: '/Slip/SalesTransactionBL041312DetailsUpdSlip3',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function (data, status, xhr) {
            hideLoading();
            if (data != null && data.webResponseCommon != null && data.webResponseCommon.returnCode == '202') {
                details.filter(x => x.recId == tInssurredPartySelectedData.recId)[0].slip3BL041312 = tStep3Table.rows().data().toArray();
                toastr.success("", "Saved");
            } else {
                toastr.warning(data.webResponseCommon.returnMessage, "UnSaved");
            }
...
$("#btnSubmitStep4").click(function () {
    ...
    $.ajax({
        type: 'POST',
        url: '/Slip/SalesTransactionBL041312DetailsUpdSlip4',
        data: formData,
        processData: false,
        contentType: false,
...
$("#btnStep4ExportData").click(function () {
    ...
    formData.append('businesscode', BusinessLineCode);
    formData.append('contactid', $("#ContactId").val());
    formData.append('businessPage', JSON.stringify("AF1_04"));
...
$('#fileInputStep5').change(function () {
    ...
    if (fileName != '') {
        ...
        $.ajax({
            type: 'POST',
            url: '/ExcelImport/ImportExcelAF1_04_DetailsStep5',
            data: formData,
            processData: false,
            contentType: false,
            beforeSend: function (xhr) {
...
$("#btnPrintCq").click(function () {
    var formData = new FormData();
    formData.append('businessPage', JSON.stringify("CQ_12"));
    formData.append('businesscode', BusinessLineCode);
