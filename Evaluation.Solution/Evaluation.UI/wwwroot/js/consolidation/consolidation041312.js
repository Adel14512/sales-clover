...
$(document).ready(function () {
    ...
    $('#tPolicies tbody').on('click', 'tr', function () {
        ...
        //tPolicies.$('tr.consolidationselectedrow').removeClass('selected');
        ...
        fileSignedOffer = policyRelatedDoc.find(x => x.documentType == "SignedOffer" && x.policyId == policySelectedData.policyId);
        fileProposalForm = policyRelatedDoc.find(x => x.documentType == "ProposalForm" && x.policyId == policySelectedData.policyId);
        filePassportId = policyRelatedDoc.find(x => x.documentType == "PassportId" && x.policyId == policySelectedData.policyId);
        fileInsuranceCard = policyRelatedDoc.find(x => x.documentType == "InsuranceCard" && x.policyId == policySelectedData.policyId);
        filePolicyDocument = policyRelatedDoc.find(x => x.documentType == "PolicyDocument" && x.policyId == policySelectedData.policyId);
        fileCommissionSheet = policyRelatedDoc.find(x => x.documentType == "CommissionSheet" && x.policyId == policySelectedData.policyId);
        ...
        var tPrice = $('#tPrice').DataTable({
            destroy: true,
            data: paymentList,//.filter(x => x.policyId == policySelectedData.policyId),
            responsive: true,
            autoWidth: true,
            columns: [
                { data: 'paymentNumberStr', title: 'Payment No.', },
    ...
    var tInsured = $('#tInsured').DataTable({
        data: salesTransactionBL041312.aF1BL041312,
        responsive: true,
        autoWidth: true,
        ordering: false,
        columns: [
    ...
    function createUpdateConsolidation(model) {
        ...
        if (modelfile == null) {
            ...
            $.ajax({
                type: 'POST',
                url: '/Consolidation/CreateConsolidationDocument041312',
                data: formData,
                processData: false,
                contentType: false,
            ...
            $.ajax({
                type: 'POST',
                url: '/Consolidation/UpdateConsolidationDocument041312',
                data: formData,
                processData: false,
                contentType: false,
                beforeSend: function (xhr) {
    ...
    function SubmitAf32(isClientMaster) {
        if (isClientMaster) {
            ...
            $.ajax({
                url: "/Business/EditAF1_04Consolidation",
                type: "POST",
                contentType: false,
                processData: false,
                data: formBody,
            ...
            if ($("#form").valid()) {
                ...
                var formBody = new FormData();
                var af8Obj = {};
                af8Obj.ResidenceCode = salesTransactionBL041312.aF1BL041312.residenceCode;
                af8Obj.NetworkLevelCode = salesTransactionBL041312.aF1BL041312.networkLevelCode;
                af8Obj.ClassOfCoveragCode = salesTransactionBL041312.aF1BL041312.classOfCoveragCode;
                af8Obj.Ambulatory = salesTransactionBL041312.aF1BL041312.ambulatory;
                af8Obj.PrescriptionMedecine = salesTransactionBL041312.aF1BL041312.prescriptionMedecine;
                af8Obj.DoctorVisit = salesTransactionBL041312.aF1BL041312.doctorVisit;
                af8Obj.NSSF = salesTransactionBL041312.aF1BL041312.nssf;
                af8Obj.AF1BL041312List = getValues();
                obj.AF1BL041312 = af8Obj;
...
function getValues() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var rowValues = {
            ...
            genderCode: cells[3].dataset.gendercode,
            dob: cells[4].dataset.dob,
            nationalityCode: cells[5].dataset.nationalitycode,
            maritalStatusCode: cells[6].dataset.maritalstatuscode,
            mobileNumber: cells[7].textContent,
...
function Submit() {
    ...
    formBody.append("data", JSON.stringify(obj));
    $.ajax({
        url: "/Consolidation/UpdateConsolidationForm041312",
        type: "POST",
        contentType: false,
        processData: false,
        data: formBody,
        ...
        success: function (data) {
            ...
            if (data != null && data.webResponseCommon.returnCode == '202') {
                ...
                policy.vatOnCommisionPer = parseFloat($("#VATOnCommisionPer").text()) || 0;
                policy.vatOnCommisionAmount = parseFloat($("#VATOnCommisionAmount").text()) || 0;
                policy.builtInFixedTaxesAmount = parseFloat($("#BuiltInFixedTaxesAmount").text()) || 0;
                policy.builtInPropTaxesPer = parseFloat($("#BuiltInPropTaxesPer").text()) || 0;
                policy.builtInPropTaxesAmount = parseFloat($("#BuiltInPropTaxesAmount").text()) || 0;
                policy.builtInCostOfPolicyAmount = parseFloat($("#BuiltInCostOfPolicyAmount").text()) || 0;
                policy.insurerLoadingPer = parseFloat($("#InsurerLoadingPer").text()) || 0;
                policy.insurerLoadingAmount = parseFloat($("#InsurerLoadingAmount").text()) || 0;
                policy.NetPremiumAmount = parseFloat($("#netPremium").text()) || 0;
                policy.policyNumber = $("#txtPolicyNumber").val();
                policy.policyEffectiveDate = $("#txtEffectiveDate").val();
                policy.policyExpiryDate = $("#txtExpiryDate").val();
                policy.policyIssuedDate = $("#txtEmissionDate").val();
