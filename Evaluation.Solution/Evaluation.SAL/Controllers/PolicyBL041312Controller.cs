...
namespace Evaluation.SAL.Controllers
{
    ...
    public class PolicyBL041312Controller : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult PolicyBL041312FindAllWithParentPolicyId([FromBody] PolicyBL041312FindParentPolicyIdRecReq policyBL041312FindParentPolicyIdRecReq)
        {
            PolicyBL041312Resp resp;
            resp = new()
            {
            ...
            try
            {
                if (!ModelState.IsValid)
                {
                    ...
                    resp.WebResponseCommon.ReturnMessage = "Bad Request;" + string.Join(" | ", ModelState.Values
                        .SelectMany(v => v.Errors)
                        .Select(e => e.ErrorMessage));
                    resp.WebResponseCommon.CorrelationId = policyBL041312FindParentPolicyIdRecReq == null ? Guid.NewGuid().ToString() :
                        policyBL041312FindParentPolicyIdRecReq.WebRequestCommon == null ? Guid.NewGuid().ToString() :
                        policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.CorrelationId == null ? Guid.NewGuid().ToString() :
                        policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.CorrelationId.Trim() != string.Empty ?
                        policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.CorrelationId : Guid.NewGuid().ToString();

                    return Ok(resp);
                }
                _logger.SetCorrelationId(policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.CorrelationId, policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.UserName);
                _logger.LogInfo("Calling the Endpoint PolicyFindAllWithParentPolicyId is started");
                _logger.LogDebug(JsonConvert.SerializeObject(policyBL041312FindParentPolicyIdRecReq));
                resp.Policy = InstManagerAccessPoint.GetNewAccessPoint().PolicyBL041312FindAllWithParentPolicyId(policyBL041312FindParentPolicyIdRecReq.ParentPolicyId);
                if (resp.Policy == null)
                    return Ok(resp);
                if (resp.Policy.Count == 0)
                    return Ok(resp);
                resp.SalesTransactionBL041312 = InstManagerAccessPoint.GetNewAccessPoint().
                    SalesTransactionBL041312FindAF1WithRecIdFilter(resp.Policy[0].SalesTransactionId);
                resp.paymentScheduleBL041312 = InstManagerAccessPoint.GetNewAccessPoint().
                   PolicyPaymentScheduleBL041312(resp.Policy[0].SalesTransactionId, resp.Policy[0].BusinessLineCode);
                resp.policyRelatedDoc041312 = InstManagerAccessPoint.GetNewAccessPoint().
                   PolicyRelatedDocBL041312(resp.Policy[0].SalesTransactionId, resp.Policy[0].BusinessLineCode);
                ...
                if (resp != null)// && resp.Region.Count == 1)
                {
                    ...
                    resp.WebResponseCommon.ReturnCode = resp.Policy[0].Reserved1 != null ? StatusCodes.Status302Found.ToString() : StatusCodes.Status201Created.ToString();
                    resp.WebResponseCommon.CorrelationId = policyBL041312FindParentPolicyIdRecReq.WebRequestCommon.CorrelationId;
                    if (resp.Policy[0].Reserved1 != null)
                        resp.Policy = new List<PolicyBL041312Dto>() { };
                }
                else
